...
@celery_app.task(name="revision.run_revision", bind=True, max_retries=1)
def run_revision(self, job_id: int) -> dict:
...
async def _run_revision(job_id: int) -> dict:
    ...
    async with AsyncSessionLocal() as db, CancellationToken(job_id) as token:
        ...
        from app.features.documentation.workflows.revision_workflow import RevisionWorkflow
