...
`GET /projects/{id}/features` answers "what can *this* project do", which needs a
...
@router.get("", response_model=list[FeatureCatalogItem])
async def list_features(user: CurrentUser) -> list[FeatureCatalogItem]:
    """Every feature, built or planned, with no project in the picture."""
    return [
        FeatureCatalogItem(
            id=f.id,
            label=f.label,
