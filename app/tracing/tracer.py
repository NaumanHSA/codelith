...
class _NoOpStepContext:
    ...
    def stream(self, message: str, **data: Any) -> None:
        return None
...
class Tracer:
    ...
    def _format_message(self, msg: str, tag: str) -> str:
        """
        Prefix messages with optional agent tag.
        Keeps output compact and avoids leading blank lines unless explicitly requested.
        """
        tag = "" if tag in {"[None]", "[]", "None"} else tag
        msg = msg.strip()
        if tag:
            return f"{tag} {msg}"
        return msg
