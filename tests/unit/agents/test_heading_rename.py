...
def agent(monkeypatch) -> ReviserAgent:
    ...
    monkeypatch.setattr("app.agents.composition.reviser.SectionContextBuilder", _Builder)
    monkeypatch.setattr("app.agents.composition.reviser.save_artifact", lambda *a, **k: None)
    monkeypatch.setattr("app.agents.composition.reviser.save_input_artifact", lambda *a, **k: None)
    monkeypatch.setattr("app.agents.composition.reviser.save_text_artifact", lambda *a, **k: None)
    return a
...
class TestLinksFromElsewhereInTheSite:
    ...
    def sweep(self, monkeypatch):
        def _run(pages):
            db = _SweepDB([])
            monkeypatch.setattr(
                "app.services.revision_service.DocSiteRepository",
                lambda _db: SimpleNamespace(
                    get_for_project=lambda pid: _async(SimpleNamespace(id=1))
                ),
            )
            monkeypatch.setattr(
                "app.services.revision_service.DocPageRepository",
