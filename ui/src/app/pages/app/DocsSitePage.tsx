...
import WriteBar from '../../components/docs/WriteBar'
...
/** `generating` holds a page address while one page is being written. A run
 *  started from the selection covers many, so it takes a name no address can
 *  collide with. */
const SELECTION_RUN = '\u0000selection'
...
export default function DocsSitePage() {
  ...
  /** Pages ticked for the next run, by address. Compose used to own this on a
   *  page of its own; it belongs here, next to the docs it writes into. */
  ...
  const compose = async (slugs: string[], mark: () => void, clear: () => void) => {
    ...
    try {
      const job = await api.compose(id, {
        page_slugs: slugs,
        // Markdown always. Format is a property of leaving, chosen on export from
        // the same stored text — asking for it before a word is written meant the
